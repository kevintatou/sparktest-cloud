...
function SuiteSection({ definitions, suites, createSuite, deleteSuite }: Pick<Props, 'definitions' | 'suites' | 'createSuite' | 'deleteSuite'>) {
  ...
  return <ResourceLayout title="Test Suites" description="Group definitions into sequential or parallel runs.">
    <Card><CardContent className="pt-6"><form onSubmit={submit} className="space-y-5">
      <div className="grid gap-4 md:grid-cols-3"><div className="space-y-2"><Label htmlFor="suite-name">Name</Label><Input id="suite-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Smoke tests" /></div><div className="space-y-2"><Label htmlFor="suite-description">Description</Label><Textarea id="suite-description" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Optional" rows={1} /></div><div className="space-y-2"><Label htmlFor="suite-mode">Execution mode</Label><select id="suite-mode" className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={mode} onChange={(event) => setMode(event.target.value as Suite['executionMode'])}><option value="sequential">Sequential</option><option value="parallel">Parallel</option></select></div></div>
      <div className="space-y-2"><Label>Definitions</Label>{definitions.length === 0 ? <p className="text-sm text-muted-foreground">Create a definition first.</p> : <div className="grid gap-2 md:grid-cols-2">{definitions.map((definition) => <label key={definition.id} className="flex items-center gap-2 rounded-md border border-border p-3 text-sm"><Checkbox checked={selected.includes(definition.id)} onCheckedChange={(checked) => setSelected((current) => checked ? [...current, definition.id] : current.filter((id) => id !== definition.id))} />{definition.name}</label>)}</div>}</div>
...
function Empty({ text }: { text: string }) { return <p className="rounded-lg border border-dashed border-border p-8 text-center text-sm text-muted-foreground">{text}</p>; }
