...
import {
  ArrowLeft, Play, RotateCcw, Clock, Calendar, Hash,
  CheckCircle2, XCircle, Loader2, AlertCircle, Timer,
  Terminal, Box
} from 'lucide-react';
...
function StatusBadge({ status }: { status: Run['status'] }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${classes}`}>
      {icon}
...
export function RunDetail({ run, definition, onBack, onRetry }: RunDetailProps) {
  const isTerminal = ['completed', 'passed', 'failed', 'error', 'cancelled'].includes(run.status);
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-mono font-medium truncate max-w-[180px]" title={run.id}>
                  {run.id.slice(0, 12)}…
                </p>
              </div>
            </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-mono font-medium truncate max-w-[180px]" title={run.image}>
                  {run.image}
                </p>
              </div>
            </div>
            ...
            <div className="text-center py-12 text-muted-foreground">
              {run.status === 'running' ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p>Run is queued. Output will appear once an agent picks it up.</p>
                </div>
              ) : (
