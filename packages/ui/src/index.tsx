...
export const TestDefinitionCard: React.FC<TestDefinitionCardProps> = ({
  ...
  const getLanguageClasses = (image: string) => {
    const classes = {
      ...
      python: 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800',
      rust: 'bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-800',
      default: 'bg-gray-50 dark:bg-gray-900/20 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-800'
    };
  ...
  return (
    <div className="bg-card rounded-lg border p-4 transition-all duration-200 hover:shadow-md">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <span className={`px-2 py-1 rounded text-xs font-medium border ${getLanguageClasses(definition.image)}`}>
          {definition.image}
        </span>
      </div>
...
export const ExecutorCard: React.FC<ExecutorCardProps> = ({
  ...
  const getTypeClasses = (image: string) => {
    const classes = {
      'local-runner': 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300',
      'k8s-runner': 'bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300',
      'docker-runner': 'bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-300',
      default: 'bg-gray-50 dark:bg-gray-900/20 text-gray-700 dark:text-gray-300'
    };
    return classes[image as keyof typeof classes] || classes.default;
  };
    ...
    <div className="bg-card rounded-lg border p-4 transition-all duration-200 hover:shadow-md">
      <div className="flex items-start justify-between mb-3">
        <div className="space-y-2 flex-1">
          ...
          <div className="flex gap-2 flex-wrap">
            <span className={`px-2 py-1 rounded text-xs font-medium ${getTypeClasses(executor.image)}`}>
              {executor.image}
            </span>
          </div>
        </div>
    ...
    <div className="bg-card rounded-lg border p-4 transition-all duration-200 hover:shadow-md">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
        <div className="flex items-center gap-1">
          ...
          <span>{suite.testDefinitionIds.length} test{suite.testDefinitionIds.length !== 1 ? 's' : ''}</span>
        </div>
        <span>•</span>
